...
class MuskensNet(tf.keras.layers.Layer):
  ...
  def call(self, inp, weights):
    ...
    x = tf.nn.conv2d(input=x, filters=weights['last_layer_conv'], strides=[1,1,1,1], padding='SAME')
...
def outer_train_step(inp, model, optim, meta_batch_size=25, num_inner_updates=1):
  ...
  print("type(grad[0]), ", type(gradients[0]))
  print("len of grad: ", len(gradients))
...
run_results = run_maml(n_way=1, k_shot=2, inner_update_lr=.0004, num_inner_updates=1, meta_batch_size=1, meta_train_iterations=50, learn_inner_update_lr=False)
